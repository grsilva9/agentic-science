...
class ConvBnSiLu(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride = 1, padding = 0):
        ...
        """
        Input description.
        Basic convolution block, Batchnormalization and SiLU actication function.
...
class Unet(nn.Module):
    ...
    def __init__(self, timesteps, time_embedding_dim, in_channels = 3, out_channels = 2, base_dim = 32, dim_mults = [2, 4, 8, 16]):
        ...
        channels = self.cal_channels(base_dim, dim_mults) #. Defines sequence of channels.

        self.init_conv = ConvBnSiLu(in_channels, base_dim, 3, 1, 1) #. An initial convolutional blcok (+ Batch Norm and Silu activation function.)
        self.time_embedding = nn.Embedding(timesteps, time_embedding_dim) #. Simple embedding in higher dimension. Weights W are learned as parameters.

        #. Model architecture.

        self.encoder_blocks = nn.ModuleList([ EncoderBlock(c[0], c[1], time_embedding_dim) for c in channels ]) 
        self.decoder_blocks = nn.ModuleList([ DecoderBlock(c[1], c[0], time_embedding_dim) for c in channels[::-1]]) #. Mirrored version.
    ...
    """
                            ...
                            FINISH Understanding. You should start implementing from the residual connections. 
    
    """

 
    def cal_channels(self, base_dim, dim_mults):

        dims = [base_dim * x for x in dim_mults] # Expands dimensions.
        dims.insert(0, base_dim) #. Inserts 0 dimension.

        channels = []

        for i in range(len(dims) -1):
            channels.append((dims[i], dims[i + 1])) #. Produces a list of in-channels and out channels.
